...
$sql = "SELECT * FROM datalist WHERE slug = '$slug'";
...
$flag = "/flags/1x1/$country_code.svg";
} else {
   header("Location: /404");
die();
}
      ...
      <div class="  row align-items-center">
         <div class="col-sm-6">
            <div class="d-flex align-items-center">
               ...
               <div>
                  <div class="d-flex align-items-center"> <h1 class="m-0 p-0 h3 ms-2"><?=ucwords($u_title)?></h1> <a class="btn-sm btn btn-warning small ms-2 m-0" href="#more" role="button">More</a> </div>
...
<div class="d-flex justify-content-between"> 
  <button class="codehap_slide_btn bg-warning rounded-circle fs-1 ps-4 pe-4 pt-2 pb-2 text-white" id="slideLeft" type="button">
   <!-- <?=$svg_left?> -->
   <-
</button>
    <button class="codehap_slide_btn bg-warning rounded-circle fs-1 ps-4 pe-4 pt-2 pb-2 text-white" id="slideRight" type="button">
      <!-- <?=$svg_right?> -->
   ->
   </button>
</div>
...
<ul class="list-group m-1" style="width:250px;">
  ...
  $count_hastags = 1;
  foreach (array_filter($value['data']) as $data) {
  $data = explode('@2@',$data);
  $hashtag = '';
  $hashtag_count = '10K+';
  if($data[0]){
...
<div class="mt-5 p-2">
            <div class="h5 ">More From: <strong><?=$country?></strong></div>
            <?php 
               $sql = "SELECT * FROM datalist WHERE country LIKE '%$country2%'";
               $result = $conn->query($sql);
               if ($result->num_rows > 0) {
               ...
               $slug = $row["slug"];
               $city = $row["city"];
               echo '<a class="badge text-dark" href="'. $slug.'">'. ucwords($city) .'</a>';
                 }
               } else {
                 echo "0 results";
               }
